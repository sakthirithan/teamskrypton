...
export function BulkEntryCreation({ session }: BulkEntryCreationProps) {
  ...
  const handleBulkCreate = async () => {
    if (formData.selectedUsers.length === 0 || !formData.skill_name) {
      toast({
        variant: 'destructive',
        title: 'Validation Error',
        description: 'Please select users and enter skill name'
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: `Created ${entriesToCreate.length} pending entries`
      });
  ...
  if (!isLeadership || session.status === 'closed') {
    return null;
  }
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4 pt-4">
          ...
          <Button 
            ...
            className="w-full"
            disabled={isCreating || formData.selectedUsers.length === 0 || !formData.skill_name}
          >
